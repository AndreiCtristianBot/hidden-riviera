...
import { logRecommendation } from "./data/recommendationLogs";
import { ensureAppStyles, ensureFonts } from "./styles/appStyles";
import { fetchPlaces } from "./data/placesApi";
...
export default function App() {
  ...
  const tog = (id) =>
    setVibes((p) => (p.includes(id) ? p.filter((v) => v !== id) : [...p, id]));

  const gen = () => {
    setSc("loading");
    setTimeout(() => {
      const generated = getItinerary(vibes, dur, comp, places);
      setIt(generated);
      setSc("result");
      logRecommendation({ vibes, duration: dur, companion: comp, itinerary: generated });
    }, 3000);
  };
    ...
    <div className="shell">
      <div className="side-info">
        ...
        <h2>
          Discover the
          <br />
          <em>Hidden</em> Riviera
        </h2>
        <p>
          An AI-powered trip planner that fights overtourism on the Côte d'Azur by redirecting
          visitors to hidden gems — away from the crowds.
        </p>
        <p style={{ fontSize: 13, color: "rgba(255,255,255,.35)" }}>
          Built by a 17 y/o from Romania 🇷🇴❤️🇫🇷
          <br />
      ...
      <div className="app">
        {sc !== "splash" && sc !== "result" && (
          <div className="prog">
            <div className="prog-f" style={{ width: `${pr}%` }} />
          </div>
        )}
        ...
        {sc === "vibe" && (
          <>
            ...
            {vibes.length > 0 && (
              <div className="bot">
                <button className="btn btn-g" onClick={() => setSc("duration")}>
                  Continue →
                </button>
              </div>
            )}
        ...
        {sc === "duration" && (
          <>
            ...
            {dur && (
              <div className="bot">
                <button className="btn btn-g" onClick={() => setSc("companion")}>
                  Continue →
                </button>
              </div>
            )}
        ...
        {sc === "companion" && (
          <>
            ...
            {comp && (
              <div className="bot">
                <button className="btn btn-g" onClick={gen}>
